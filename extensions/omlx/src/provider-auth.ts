...
import { OMLX_LOCAL_API_KEY_PLACEHOLDER } from "./defaults.js";

export function hasOmlxAuthorizationHeader(headers: unknown): boolean {
...
export function resolveOmlxProviderAuthMode(
  ...
  if (normalized !== undefined) {
    ...
    if (
      !trimmed ||
      trimmed === OMLX_LOCAL_API_KEY_PLACEHOLDER ||
...
export function shouldUseOmlxApiKeyPlaceholder(params: {
  hasModels: boolean;
...
export function shouldUseOmlxSyntheticAuth(
  providerConfig: ModelProviderConfig | undefined,
): boolean {
  const hasModels = Array.isArray(providerConfig?.models) && providerConfig.models.length > 0;
  return (
    hasModels &&
    !resolveOmlxProviderAuthMode(providerConfig?.apiKey) &&
    !hasOmlxAuthorizationHeader(providerConfig?.headers)
  );
}
