// Omlx plugin module implements runtime behavior.
import {
...
import { hasOmlxAuthorizationHeader } from "./provider-auth.js";

type OmlxAuthHeadersParams = {
  apiKey?: string;
  json?: boolean;
  headers?: Record<string, string>;
};

export function buildOmlxAuthHeaders(
  params: OmlxAuthHeadersParams,
): Record<string, string> | undefined {
  const headers: Record<string, string> = { ...params.headers };
  // Runtime auth resolution is strict, but guard known non-secret markers here.
  const apiKey = params.apiKey?.trim();
  const isSyntheticLocalKey = apiKey === OMLX_LOCAL_API_KEY_PLACEHOLDER;
...
export async function resolveOmlxConfiguredApiKeyForProvider(params: {
  providerId: string;
...
export async function resolveOmlxConfiguredApiKey(params: {
  config?: OpenClawConfig;
  env?: NodeJS.ProcessEnv;
  path?: string;
  allowUnresolved?: boolean;
}): Promise<string | undefined> {
  return await resolveOmlxConfiguredApiKeyForProvider({
    ...params,
    providerId: OMLX_PROVIDER_ID,
  });
}

export async function resolveOmlxProviderHeaders(params: {
  config?: OpenClawConfig;
  env?: NodeJS.ProcessEnv;
  headers?: unknown;
  path?: string;
  ...
  const pathPrefix = params.path ?? "models.providers.omlx.headers";
...
/**
 * Resolves oMLX API key and provider headers in parallel.
 * Use this as the standard auth setup step before discovery or model load calls.
 */
export async function resolveOmlxRequestContext(params: {
  config?: OpenClawConfig;
  agentDir?: string;
  env?: NodeJS.ProcessEnv;
  ...
  const providerHeaders =
    params.providerHeaders ?? params.config?.models?.providers?.[OMLX_PROVIDER_ID]?.headers;
...
/**
 * Resolves oMLX runtime API key from config.
 */
export async function resolveOmlxRuntimeApiKey(params: {
  config?: OpenClawConfig;
  agentDir?: string;
  env?: NodeJS.ProcessEnv;
  headers?: unknown;
  ...
  const providerHeaders = params.headers ?? config.models?.providers?.[OMLX_PROVIDER_ID]?.headers;
  const hasAuthorizationHeader = hasOmlxAuthorizationHeader(providerHeaders);
  let configuredApiKeyPromise: Promise<string | undefined> | undefined;
  const getConfiguredApiKey = async () => {
    configuredApiKeyPromise ??= resolveOmlxConfiguredApiKey({
      config,
      env: params.env,
  ...
  const resolveConfiguredApiKeyOrThrow = async () => {
    ...
    const envMarker = `\${${OMLX_DEFAULT_API_KEY_ENV_VAR}}`;
    throw new Error(
      [
        "oMLX API key is required.",
        `Set models.providers.omlx.apiKey (for example "${envMarker}")`,
        'or run "openclaw models auth omlx".',
      ].join(" "),
    );
  };
